...
// 声明全局变量
var videoElement;
let containerDiv, statusBarDiv, audioDiv, timeDiv, appDiv;
...
function displayAppContent(type) {
    ...
    fetchHtmlContent(type, function (htmlContent) {
        if (appDiv.style.display === 'block' && appDiv.innerHTML === htmlContent) {
            appDiv.style.cssText = '';
            appDiv.innerHTML = '';
            applyStyles(appDiv, styles.appDiv); // 应用默认样式
        } else {
            appDiv.style.cssText = '';
...
function fetchHtmlContent(type, callback) {
    setTimeout(function () {
        ...
        switch (type) {
            case 'speed':
                htmlContent = '<div class="app-content speed-app">这是倍速控制功能的界面内容。</div>';
                break;
            case 'volume':
